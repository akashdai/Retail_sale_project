...
select count(distinct customer_id) as uniq_customer from retail_sales
...
select category, sum(total_sale) as category_sale from retail_sales
group by category
...
select to_char(sale_date, 'month'), avg(total_sale) from retail_sales
...
group by to_char(sale_date, 'month') 
order by to_char(sale_date, 'month') desc
